...
//
import { deleteEntity, getEntityData, getUserInfo, registerEntity, setUserRole, updateEntity, getFilterEntityData, getFilterEntityCount } from "../../../endpoints.js";
import { drawTagsIntoTables, inputObserver, inputSelect, CloseDialog, filterDataByHeaderType, getVerifyUsername, pageNumbers, fillBtnPagination, userPermissions, currentDateTime, eventLog, getNothing } from "../../../tools.js";
        ...
        const renderInterface = async (entities) => {
            ...
            this.entityDialogContainer.innerHTML = `
                ...
                <div class="entity_editor_header">
                    ...
                    <h1 class="entity_editor_title">Registrar <br><small>Guardia</small></h1>
                    </div>
        ...
        const reg = async (raw) => {
            registerEntity(raw, 'User')
                ...
                setTimeout(async () => {
                    ...
                    eventLog('INS', 'GUARDIA', `${parse.username}`, '');
                    const tableBody = document.getElementById('datatable-body');
                        ...
                        elem.forEach((el) => {
                            registerEntity(el, 'User')
                                ...
                                setTimeout(async () => {
                                    ...
                                    eventLog('INS', 'GUARDIA', `${parse.username}, importación`, '');
                                    const tableBody = document.getElementById('datatable-body');
                                    const container = document.getElementById('entity-editor-container');
                                    new CloseDialog().x(container);
                ...
                deleteButton.onclick = async () => {
                    ...
                    if (data.userState.name == "Disponible") {
                        deleteEntity('User', entityId)
                            ...
                            setTimeout(async () => {
                                //let data = await getUsers();
                                eventLog('DLT', 'GUARDIA', `${entityName}`, '');
                                const tableBody = document.getElementById('datatable-body');
                        ...
                        let message;
                        let status1 = await getNothing("crewOne.username", entityName, "Crew");
                        let status2 = await getNothing("crewTwo.username", entityName, "Crew");
                        let status3 = await getNothing("crewThree.username", entityName, "Crew");
                        let status4 = await getNothing("crewFour.username", entityName, "Crew");
                        let status5 = await getNothing("crewFive.username", entityName, "Crew");
