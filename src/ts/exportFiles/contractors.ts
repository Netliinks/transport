export const exportContractorPdf = (ar: any) => {
    ...
    // Save the PDF
    var d = new Date()
    var title = "log_Contratistas_"+ d.getDate() + "_" + (d.getMonth()+1) + "_" + d.getFullYear() +`.pdf`;
    doc.save(title);

}

export const exportContractorCsv = (ar: any) => {
    let rows = [];
    for(let i=0; i < ar.length; i++){
        let user = ar[i]
        // @ts-ignore
...
export const exportContractorXls = (ar: any) => {
    let rows = [];
    for(let i=0; i < ar.length; i++){
        let user = ar[i]
        // @ts-ignore
