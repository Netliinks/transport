export const exportEmployeePdf = (ar: any) => {
    ...
    for (let i = 0; i < ar.length; i++) {
        ...
        doc.text(280, row, `${user?.permitVisit ? 'Si' : 'No'}`)
        row += 5
        let limitLineas = 33
    ...
    // Save the PDF
    var d = new Date()
    var title = "log_Empleados_"+ d.getDate() + "_" + (d.getMonth()+1) + "_" + d.getFullYear() +`.pdf`;
    doc.save(title);

}

export const exportEmployeeCsv = (ar: any) => {
    let rows = [];
    for(let i=0; i < ar.length; i++){
        let user = ar[i]
        // @ts-ignore
        let obj = {
            ...
            "Visita": `${user?.permitVisit ? 'Si' : 'No'}`
        }
        rows.push(obj)
    
    }
    generateFile(rows, "Empleados", "csv");
}

export const exportEmployeeXls = (ar: any) => {
    let rows = [];
    for(let i=0; i < ar.length; i++){
        let user = ar[i]
        // @ts-ignore
    ...
    generateFile(rows, "Empleados", "xls");
}
