...
import { drawTagsIntoTables, inputObserver, inputSelect, CloseDialog, filterDataByHeaderType, getVerifyEmail, pageNumbers, fillBtnPagination, userPermissions, currentDateTime, eventLog, getSearch, getNothing } from "../../tools.js"
    ...
    private register() {
        ...
        const renderInterface = async (entities: string): Promise<void> => {
            ...
            registerButton.addEventListener('click', async() => {
                ...
                let searchExist = await getSearch("licensePlate", inputsCollection.plate.value.toUpperCase(), "Weapon")
        ...
        const UUpdate = async (entityId: any, data: any): Promise<void> => {
            ...
            updateButton.addEventListener('click', async () => {
                ...
                if($value.plate.value.toUpperCase() != data.licensePlate){
                    searchExist[1] = await getSearch("licensePlate", $value.plate.value.toUpperCase(), "Weapon")
                }
                // @ts-ignore
                ...
                updateEntity('Weapon', entityId, raw)
                    .then((res) => {
                        setTimeout(async () => {
                            ...
                            let parse = JSON.parse(raw);
                            eventLog('UPD', 'ARMA', `${parse.name} [${parse.licensePlate}]`, '')
                            new CloseDialog()
                                .x(container =
                                    document.getElementById('entity-editor-container')
                                )
    ...
    private remove() {
        ...
        remove.forEach((remove: InterfaceElement) => {
            ...
            remove.addEventListener('click', async (): Promise<void> => {
                const data: any = await getEntityData('Weapon', entityId)
                ...
                deleteButton.onclick = async () => { 
                    if(data.weaponState.name == "Disponible" || data.weaponState.name == "No disponible"){
                        ...
                        let message: any
                        let status1 = await getSearch("weaponOne.id", entityId, "Crew")
                        let status2 = await getSearch("weaponTwo.id", entityId, "Crew")
                        let status3 = await getSearch("weaponThree.id", entityId, "Crew")
                        let status4 = await getSearch("weaponFour.id", entityId, "Crew")
                        let status5 = await getSearch("weaponFive.id", entityId, "Crew")
                        if(status1 != undefined){
                            ...
                            message = {
                                role: "SEGUNDERO",
                                crew: status2.name,
                                status: status2.crewState.name
                            }
                        }else if(status3 != undefined){
                            message = {
                                role: "CUSTODIO 1",
                                crew: status3.name,
                                status: status3.crewState.name
                            }
                        }else if(status4 != undefined){
                            message = {
                                role: "CUSTODIO 2",
                                crew: status4.name,
                                status: status4.crewState.name
                            }
                        }else if(status5 != undefined){
                            message = {
                                role: "CUSTODIO 3",
                                crew: status5.name,
                                status: status5.crewState.name
                            }
                        }
                        alert(`El arma ${entityName} está asignada al ${message.role} en la patrulla: ${message.crew}, que está en estado: ${message.status}`)
                    }
                }
